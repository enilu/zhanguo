...
小说章节正文只保留故事内容，不要在 `docs/chapter-XX-draft.md` 末尾加入 `本回小结`、审读说明、承接提示、写作备注等元信息。此类内容如确有保留必要，应单独存放在正文之外的文件中，且不得混入章节正文。
继续写作或回修章节正文前，优先参照 `docs/review/chapter-prose-guardrails.md`，重点避免“评论稿化”、人物声口趋同、叙述者过度解题，以及章末直接盖棺式总结。
